...
// Tested TOS values in this scenario
uint8_t tosValue1 = 0x70; //AC_BE
uint8_t tosValue2 = 0x28; //AC_BK
uint8_t tosValue3 = 0xb8; //AC_VI
uint8_t tosValue4 = 0xc0; //AC_VO

std::string outputTOS = "tos-out.csv";
...
int main(int argc, char *argv[])
{
    ...
    int warmupTime = 1;
    int packetSize = 1472;
    std::string outputCsv = "tos-ms-2.csv";
    std::string outputTOS = "tos-out.csv";

    // Number of stations with specified tos code
    int numBE = 0;
    int numBK = 0;
    int numVI = 0;
    int numVO = 0;
    ...
    cmd.AddValue("numBE", "Number of sta with best-effort tos", numBE);
    cmd.AddValue("numBK", "Number of sta with background tos", numBK);
    cmd.AddValue("numVI", "Number of sta with video tos", numVI);
    cmd.AddValue("numVO", "Number of sta with voice tos", numVO);
    cmd.AddValue("csv", "Output csv name", outputCsv);
    cmd.Parse(argc, argv);

    int APs = countAPs(layers);
    int stations = (numBE + numBK + numVI + numVO) / APs;

    if ((numBE + numBK + numVI + numVO) % APs != 0)
    {
        std::cout << "Wrong number of stations with specified tos codes! (" << (numBE + numBK + numVI + numVO) << " stations provided)\n";
        exit(0);
    }

    // Total number of STA
    int STAs = stations * APs;
    std::cout << "There are " << STAs << " stations overall \n";

    /* Randomizing tos codes 1-4 */
    double min = 0.0;
    double max = 4.1;
    Ptr<UniformRandomVariable> a = CreateObject<UniformRandomVariable>();
    a->SetAttribute("Min", DoubleValue(min));
    a->SetAttribute("Max", DoubleValue(max));

    //uint32_t  los1 = a->GetInteger(1,4);
...
wifiPhy.Set ("EnergyDetectionThreshold", DoubleValue (-79 + 3)); */
    ...
    myfileTOS.open(outputTOS, ios::app);
    myfileTOS << ",TOS\n";
    ...
    for (int i = 0; i < APs; ++i)
    {
        for (int j = 0; j < stations; ++j)
        {
            ...
            uint32_t randm = a->GetInteger(1, 4);

            if (randm == 1 && numBE != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue1);
                numBE -= 1;
                //std::cout << "BE traffic \n";
            }
            else if (randm == 1 && numBK != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue2);
                numBK -= 1;
            }
            else if (randm == 1 && numVI != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue3);
                numVI -= 1;
            }
            else if (randm == 1)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue4);
                numVO -= 1;
            }
            ...
            else if (randm == 2 && numBE != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue1);
                numBE -= 1;
                //std::cout << "BE traffic \n";
            }
            else if (randm == 2 && numVI != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue3);
                numVI -= 1;
            }
            ...
            else if (randm == 3 && numBK != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue2);
                numBK -= 1;
            }
            else if (randm == 3 && numBE != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue1);
                numBE -= 1;
                //std::cout << "BE traffic \n";
            ...
            else if (randm == 4 && numBK != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue2);
                numBK -= 1;
            }
            else if (randm == 4 && numVI != 0)
            {
                installTrafficGenerator(wifiStaNodes[i].Get(j), wifiApNodes.Get(i), port++, offeredLoad, packetSize, simulationTime, warmupTime, tosValue3);
                numVI -= 1;
            }
...
void showPosition(NodeContainer &Nodes)
{
    ...
    for (NodeContainer::Iterator nAP = Nodes.Begin(); nAP != Nodes.End(); ++nAP)
    {
        ...
        std::cout << "Node " << NodeNumber << " has coordinates "
...
void installTrafficGenerator(Ptr<ns3::Node> fromNode, Ptr<ns3::Node> toNode, int port, std::string offeredLoad, int packetSize, int simulationTime, int warmupTime, uint8_t tosValue)
{
    ...
    Ptr<Ipv4> ipv4from = fromNode->GetObject<Ipv4>();
    Ipv4Address iaddr = ipv4from->GetAddress(1, 0).GetLocal();
    std::cout << iaddr << "\n";
    if (tosValue == 0x70)
    {
        myfileTOS << ",BE\n";
    }
    else if (tosValue == 0x28)
    {
        myfileTOS << ",BK\n";
    }
    else if (tosValue == 0xb8)
    {
        myfileTOS << ",VI\n";
    }
    else if (tosValue == 0xc0)
    {
        myfileTOS << ",VO\n";
    }
    myfileTOS.close();
